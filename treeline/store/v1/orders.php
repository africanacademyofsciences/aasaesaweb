...
<div id="primarycontent">
		...
		if( $totalOrders>0 ){ 
			...
			if ($_GET['action']=="download") {
				...
				$query = "SELECT 
					...
					ORDER BY so.date_order_started DESC, so.date_cart_started DESC
					";
				$csv = new CSV($query);
				if ($csv->generateCSV()) {
            ...
            <table class="treeline">
                ...
                <thead>
                    ...
                    foreach($orders as $row){
                        $total = ($row->products_total + $row->donation_total + $row->sponsorship_total + $row->events_total + $row->postandpacking);
						?>
                        <tr>
	...
	// we have an orderID to view/modify 
		...
		if( $order = $store->getOrderDetails($orderID) ){
            ...
            <div id="orderHeader">
				...
				if( is_array($basket->basket) && count($basket->basket)>0 ){ 
					...
					if( $addr = $basket->getAddress('delivery',$orderID) ){ 
                        ...
                        <? if($addr->country_id>0){ $basket->country=$addr->country_id; $basket->getPostageAndPacking(); } ?>
                    	<? 
					} 
				}
				else{ 
				...
				<div id="orderDetails">
                    ...
                    <? if($order->date_order_completed){ ?>Order completed: <?= date('H:i \o\n jS F Y',$order->f_date_order_completed) ?><br /><? } ?>
                    <? if( $basket->totals['donation']>0 || $basket->totals['sponsorships']>0 ){ ?>
                        <p id="giftaid"><strong>Gift Aid</strong>: <?= ( $order->use_gift_aid==1 ? 'YES' : 'NO' ) ?></p>
                    <? } ?>
            ...
            if( is_array($basket->basket) && count($basket->basket)>0 ){ 
                ...
                <thead>
                    ...
                    <th scope="col" class="col_total">Totals</th>
                </thead>
                <tbody>
                <? 
                foreach( $basket->basket as $row ) { 
                    //echo '<pre>'. print_r($row,true) .'</pre>';
                    $query = "SELECT price FROM store_orders_details WHERE order_id='".$orderID."' AND item_id=".$row->item_id." LIMIT 1 ";
                    //print "get price($query)<br>\n";
                    $item_price = $db->get_var($query);
                    if ($item_price==0) $item_price = $row->price; // default to current price.
                    ?>
                    <tr>
                        ...
                        <td class="col_price"><?= $basket->currency . number_format($item_price, 2) ?></td>
                        <td class="col_quantity"><?= $row->quantity ?></td>
                        <td class="col_total"><?= $basket->currency .number_format(($row->quantity*$item_price),2) ?></td>
                    </tr>
                    <? 
                } 
                ?>
                <tr>
                    <td colspan="5">Postage and Packaging</td>
                    <td class="col_total"><?= $basket->currency . number_format($basket->pandp,2) ?></td>
                </tr>
                </tbody>
                </table>
                <? 
			...
			if( $basket->totals['donation'] ){ 
                ...
                <tr class="goods_total">
                    ...
                    <td class="col_total total_big"><?= $basket->currency ?><?= ($basket->totals['donation']>0 ? $basket->totals['donation'] : 0) ?><?= ($basket->donation['frequency']==1 ? ' <span id="dFreq">(monthly)</span>' :'') ?></td>
                </tr>
                </tbody>
                </table>
                <? 
            ...
            ?>
					...
					<table class="treeline">
						...
						<tr>
							...
							<td><?= $basket->currency . $event->price ?></td>
						</tr>
						<? } ?>
						<tr class="goods_total">
            ...
            <table class="treeline" id="grand_total">
                <tbody>
                    <tr>
                        ...
                        <td class="col_total totals total_big"><?= $basket->currency . number_format($basket->getGrandTotal() + $basket->pandp,2) ?></td>
                    </tr>
                </tbody>
            </table>					
		...
		}	// End of if getorderdetails 
			
	} // End of else (we do have an order ID)
