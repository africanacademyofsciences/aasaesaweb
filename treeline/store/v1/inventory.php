...
//error_reporting(E_ALL);
	...
	$css = array('forms','tables','galleries','../store/'.$storeVersion.'/style'); // all CSS needed by this page
	...
	if ($productName>'' && $action=="images") {
		...
		$js[]="swfsetup";
		$extraOnloadJS.=' 
swfu = new SWFUpload(swfsetup(\''.$productName.'\', \'/treeline/store/'.$storeVersion.'/upload_store_process\')); 
';
	}
...
<div id="primarycontent">
            ...
            <form action="" method="post" id="products" >
                ...
                <fieldset>
                    ...
                    <fieldset id="controls">
                        ...
                        <a href="/treeline/store/<?=$storeVersion?>/inventory.php?action=organise&amp;product=<?= $product->name ?>" class="fauxButtons">Manage product images</a>
	...
	// UPLOAD IMAGES
		...
		if ($product->product_id>0) {
			...
			if (!file_exists($gallery_dir)) {
				?>
				<p>Could not create the store gallery directory - <?=$gallery_dir?></p>
				<?php
			}
			else {
					...
					<fieldset>
						<table id="variants" class="treeline">
							<caption>Edit <?= ($showVariants ? 'Variations' : 'Item Details') ?> of '<a href="<?= $storeURL . $basket->getProductURL($product->product_id)?>&amp;KeepThis=true&amp;TB_iframe=true&amp;height=520&amp;width=920" class="thickbox"><?= $product->title ?></a>'
								<?= ( file_exists($_SERVER['DOCUMENT_ROOT'] .'/silo/store/'.$storeVersion.'/'. $product->product_id .'/1.jpg') ? '<div style="background:url(/silo/store/'.$storeVersion.'/'. $product->product_id .'/1_vsm.jpg) no-repeat 50% 50%;width:40px;height:40px;float:right;margin-top:-1.2em;margin-bottom:-10px;"></div>' : '') ?>
							</caption>
							<thead>
								<tr>
									<th scope="col">ID</th>
