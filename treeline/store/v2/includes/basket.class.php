	...
	public function loadShoppingBasket( $cartID=false ){
		...
		if( $cartID ){
			...
			if( $data = $db->get_results($query) ){
				...
				//print "basket(".print_r($data, 1).")<br>\n";
				foreach ($this->basket as $row) {
					if($row->physical) $this->physicalproducts++;
				}
				if($this->physicalproducts) $this->getPostageAndPacking();
				else $this->pandp = 0;
				//print "Found (".$this->physicalproducts.") physical products pnp(".$this->pandp.")<br>\n";
