...
//// Get/set methods
		...
		if( (isset($email) && $email) && (isset($password) && $password) ){
			...
			if (!$member_id) {
				...
				if ($member_id >0) {
					...
					//print "$query<br>\n";
					if (!$db->query($query)) $member_id = 0;	// Cannot add to site, fail login
				}
			}
			if ($member_id>0) {
	...
	public function createAddress( $memberID=0, $address=false ){
		global $db;
		$addrID = 0;
		$memberID = $memberID+0;	// Make sure we at least have a 0;
		//print "cA($memberID, $address)<br>\n";
		if( is_array($address) ){
			
			$addrID = $this->addressExists($memberID, $address['house'], $address['post_code']);
			if (!$addrID) {
