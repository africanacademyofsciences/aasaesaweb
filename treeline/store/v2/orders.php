    ...
    if( !$orderID ){ 
        ...
        <form action="" method="post">
            <fieldset>
				...
				<p class="instructions">To filter orders, use the search form below'.($totalOrders>0?' or <a href="/treeline/store/'.$storeVersion.'/orders.php?action=download">download all orders as a CSV list</a>':"").'</p>
		...
		if( $totalOrders>0 ){ 
			...
			if ($_GET['action']=="download" && $_SERVER['REQUEST_METHOD']=="GET") {
				...
				if( $totalOrders>$perPage ){
					echo drawPagination($totalOrders,$perPage,$currentPage,'/treeline/store/'.$storeVersion.'/orders.php');
				}
			}
