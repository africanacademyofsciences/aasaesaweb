...
if ($_SERVER['REQUEST_METHOD']=="POST") {
	...
	if ($action=="create") {
		if ($_POST['title']) {
			$query = "INSERT INTO store_donation_campaign
				(title, added, msv, description, target) 
				VALUES
				(
	...
	// Update a campaign
		...
		$query = "UPDATE store_donation_campaign SET 
			...
			description = '".$db->escape($_POST['desc'])."',
...
if ($action=="create") {
    ...
    <fieldset>
		...
		<div class="field">
    	    ...
    	    <input type="text" name="target" id="f_target" value="'.($_POST['target']+0).'" />
		</div>

		<div class="field">
	        <label for="f_desc">Description:</label>
			<div class="ckeditor-box">
    	    <textarea type="text" name="desc" id="f_desc" style="float:left;">'.($_POST['desc']).'</textarea>
			</div>
		</div>
	...
	<fieldset>
		...
		<div class="field">
			<label for="f_embed">Embed code:</label>
			<p style="font-weight:bold;">@@CAMPAIGN-'.$cid.'@@</p>
		</div>
		
		<div class="field">
	        <label for="f_target">Target:</label>
    	    <input type="text" name="target" id="f_target" value="'.(($_POST?$_POST['target']:$camp->target)+0).'" />
		</div>

		<div class="field">
	        <label for="f_desc">Description:</label>
			<div class="ckeditor-box">
	    	    <textarea type="text" name="desc" id="f_desc">'.($_POST?$_POST['desc']:$camp->description).'</textarea>
			</div>
		</div>
