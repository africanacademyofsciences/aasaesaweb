...
foreach ($_POST as $k=>$v) {
	...
	if ($k=="gallery_id") {
		$productID=$v;
		//$query = "select product_id FROM store_products where name = '$product_name' AND msv=".$site->id;
		//print "Got prod id ($productID)";
...
if ($productID>0) {
	...
	if (file_exists($original_file)) {
		...
		if ($image_number>0) {
			...
			$save_path = $_SERVER['DOCUMENT_ROOT']."/store/images/".$productID."/";
			...
			if (!$image->write($save_file)) {
				...
				if (!$image->write($save_file)) print "Err - Failed to save large image";
					...
					$image = new Image;
					$image->loadFromFile($original_file);
					$image->crop_and_resize('280xc152');	
...
function logit($s) {
	...
	if ($gallery_debug) {
		...
		//else mail("phil.redclift@ichameleon.com", "Gallery debug info", $msg);
	}
}
