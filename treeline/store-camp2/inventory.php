<?php
	...
	if( isset($_POST) && $_POST ) {
		...
		if( $saveDetails ){
			...
			$nextsteps[] = '<li><a href="/treeline/store/inventory.php?action=add&product=new">Add a new product</a></li>';
			$nextsteps[] = '<li><a href="/treeline/store/inventory.php?action=edit">Manage products</a></li>';
		...
		if( $saveInventory ){
			...
			if( $store->updateInventory($productID,$properties) ){
				...
				$nextsteps[] = '<li><a href="/treeline/store/inventory.php?action=edit&product='.$productName.'">Edit product details</a></li>';
				$nextsteps[] = '<li><a href="/treeline/store/inventory.php?action=edit">Manage products</a></li>';
			}
			else{
				$feedback = 'error';
...
'; // extra on page CSS
	...
	if ($productName>'' && $action=="images") {
		...
		$js[]="swfsetup";
		$extraOnloadJS.=' 
swfu = new SWFUpload(swfsetup(\''.$product->product_id.'\', \'/treeline/store/upload_store_process\')); 
';
	}
					...
					<table class="tl_list">
						...
						<thead>
							...
							foreach($products as $row){ 
								...
								?>
								<tr>
									<td nowrap style="padding-right: 10px;"><a href="?product=<?= $row->name ?>"><?= $row->title ?></a></td>
									<td><?= $short_desc ?></td>
