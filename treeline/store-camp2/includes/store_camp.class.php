...
//// Get/set methods
	...
	public function loadByCat( $catName=false, $from=0, $quantity=9 ){
		...
		$query = "SELECT sp.product_id, sp.title, sp.name, sp.author,
			sp.short_desc, sp.`priority`, sp.physical, sp.page_guid,
			(
				SELECT si.price 
			...
			(
				...
				WHERE si3.product_id=sp.product_id 
				LIMIT 1
			) AS item_id,
			sp.image AS images
		...
		//if( $catName ) $query .= " AND sc.name='". $catName ."' ";

		$query .= "GROUP BY sp.product_id ";
		
		$query .= "UNION 
			SELECT 0  AS product_id, p.title, p.name, '' AS author,
			p.meta_description AS short_desc, 0 AS priority, 
			0 AS physical, p.guid AS page_guid,
			e.price,
			'0,0' AS price_range,
			e.capacity AS stock_level,
			0 AS item_id,
			'' AS images
			FROM events e
			INNER JOIN pages p ON p.guid=e.guid
			WHERE e.end_date > NOW()		
			AND p.msv = ".$site->id."
			AND e.price>0 
			";		
		$query .= "ORDER BY priority ASC, title ASC";
	...
	public function loadByName( $name=false, $useStatus=true ){
		...
		$query = "SELECT sp.product_id, sp.title, sp.name, sp.author, 
			sp.short_desc, sp.long_desc, sp.`priority`,  
			...
			(
				SELECT MAX(si.stock_level) FROM store_inventory si
				WHERE si.product_id=sp.product_id LIMIT 1
			) stock_level,
			(
				SELECT si3.item_id FROM store_inventory si3
				WHERE si3.product_id=sp.product_id LIMIT 1
			) item_id,
			(
				SELECT 
			...
			WHERE sp.msv=".$site->id."
			AND sp.name='". $name ."'
			AND sp.`status`".($useSelected?"=":">-")."1
			GROUP BY sp.product_id LIMIT 1
			";
		//print "lp($query) <Br>\n";
		//print "<!-- $query --> \n";
		if( $data = $db->get_results($query) ){
...
".$catWhere." ";
	...
	public function getProductVariants( $productID=false ){
		...
		if( $productID ){
			...
			$query = "SELECT DISTINCT si.tagline AS title, count(si.tagline) total, 
						si.tagline AS variants 
						FROM store_inventory si
						WHERE product_id=$productID
						GROUP BY si.tagline
						";
	...
	public function getItemVariants($productID=false,$itemID=false){
		...
		if( $productID && $itemID ){
			$query = "SELECT
				GROUP_CONCAT(variant_id SEPARATOR ',') 
				FROM store_products_variants 
				WHERE product_id=$productID 
				AND item_id=$itemID
				";
			//print "$query<br>\n";
	...
	public function getVariantList( $type=false ){
		...
		if( $type>0 ){
			$query = "SELECT sv.* FROM store_variants sv
				LEFT JOIN store_variants_types svt ON sv.variant_id=svt.variant_id
				WHERE sv.msv=".($site->id+0)." 
				AND svt.type_id=$type 
				ORDER BY sv.sort_order ASC, sv.variant_id ASC";
			if( $data = $db->get_results($query) ){
				return $data;
			}else{
				return false;
	...
	public function addVariant($parent=false, $title=false){
		global $db, $site;
		//print "aV($parent, $title)<br>\n";
		if( $parent && $title ){
			if( $name = $this->generateName(false,$title,'variants') ){
				$query = "INSERT INTO store_variants (title,name,msv) VALUES ('". $db->escape($title) ."','$name',".($site->id+0).")";
				//print "$query<Br>\n";
				if( $db->query($query) ){
					$varID = $db->insert_id;
	...
	public function addVariantType($title=false){
		...
		if( $title ){
			if( $name = $this->generateName(false,$title,'types') ){
				$query = "INSERT INTO store_types (title,name,msv) VALUES ('". $db->escape($title) ."','$name',".($site->id+0).")";
				if( $db->query($query) ){
					return true;
				}else{
					return false;
...
';
	...
	public function getOrders($from=0, $to=20, $filterBy=false, $filterOrder='asc', $filterValue=false){
		...
		*/
		...
		//print "got total($totalOrders)<br>\n";		
		$query .= "LIMIT $from, $to ";
	...
	public function saveProduct($productID=false, $properties=false){
		global $db, $site;
		//print "sP($productID, ".print_r($properties, true).")<br>\n";
		$this->dbg=true;
		reset($this->msg);
		
		if (!$db->escape($properties['title'])) {
			$this->msg="You must enter a name for your product";
			return false;
		}
...
//print_r($properties);
			if( $productID>0 ){
				...
				$query = "UPDATE store_products 
					SET title='". $db->escape(str_replace('"', "'", $properties['title'])) ."', 
					...
					physical=".($properties['physical']?1:0).", 
					featured=".($properties['featured']?1:0).",
				...
				if( $db->affected_rows>=0 ){
					...
					//print "category(".$properties['category'].")<br>\n";
					if( isset($properties['category']) && $properties['category'] ){
						...
						//print "sort($sort_order) $query<Br>\n";
						$query = "SELECT cat_id FROM store_categories WHERE name='". $properties['category'] ."' AND msv=".$site->id;
						//print "cat($query)<br>\n";
						$category_id = $db->get_var($query);
						if ($sort_order>0 && $category_id>0) {
							$db->query("DELETE FROM store_categories_products WHERE product_id=$productID");
							$query = "REPLACE INTO store_categories_products 
								(cat_id, product_id, sort_order) 
								VALUES 
								($category_id, $productID,".($sort_order+0).")
								";
							//print "$query<br>\n";
							$db->query($query);
						}
						else print "Failed to get sort($sort_order) category($category_id)<br>\n";
					}
				}
				else {
			...
			// No ID, you must be new around here...
				...
				if( $this->generateName(false,$properties['title'],'products') ){
					...
					$query = "INSERT INTO store_products 
						...
						'1', ".($physical+0).", ".($properties['featured']?1:0).",
	...
	public function saveImage($productID, $image) {
		...
		if (file_exists($dest)) {
			...
			if ($sz[0]==170 && $sz[1]==170 ) {
				...
				$this->errmsg[] = "Product image dimensions must be exactly 170x170 pixels";
			}
		}
		else {
	...
	public function loadImage($productID) {
		...
		if ($productID) {
			...
			if ($img = $db->get_var($query)) {
				$file = $_SERVER['DOCUMENT_ROOT']."/store/images/".$productID.$img;
				//print "file($file)<br>\n";
				if (file_exists($file)) return '<div class="fields"><label for="f_img" style="visibility:hidden;">Image</label><img id="f_img" src="/store/images/'.($productID.$img).'" /></div>';
			}
		}
	}
	...
	*/
	...
	public function generateName($ID=false,$title=false,$table='products') {
		...
		if( $title ){
			...
			$query .="AND `status`>-1 AND msv=".($site->id+0)." ";
			//print "$query<br>\n";
	...
	// Takes some paypal delivery variables, check if they have an address book entry already
	// Adds address to address book if necessary.
	public function setDeliveryAddress($member_id, $street, $city, $state, $zip, $country, $country_code) {
		global $db;
		$this->errmsg = array();
		$query="select addr_id from store_address_book WHERE member_id=$member_id AND street='$street' and post_code='$zip' limit 1";
		$this->errmsg[] = $query;
		$addr_id=$db->get_var($query);
		if ($addr_id>0) {
			$this->errmsg[]="Found addr($addr_id)";
			return $addr_id;
		}
		
		// Add this address and return the new id
		$query="INSERT INTO store_address_book (member_id, street, town_city, county, post_code, country, country_id)
			values 
			($member_id, '$street', '$city', '$state', '$zip', '$country', '$country_code')";
		$this->errmsg[] = $query;
		if ($db->query($query)) return $db->insert_id;
		$this->errmsg[] = "Failed to add delivery address";
		return 0;
	}
