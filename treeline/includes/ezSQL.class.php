<?php
	...
	*/
	...
	if ($ezCoreRoot) {
		$ezCore=$ezCoreRoot."/treeline/includes/ezSQL/ez_sql_core.php";
	}
	//print "<!-- ez core($ezCore) --> \n";
	include_once $ezCore;
	...
	*/
	...
	class ezSQL_mysql extends ezSQLcore
	{
		...
		*/
		...
		function connect($dbuser='', $dbpassword='', $dbhost='localhost')
		{
			...
			{
				print "<!-- Failed to connect to ($dbhost, $dbuser, ".substr($dbpassword, 0, 2).") --> \n";
		...
		*/
		...
		function query($query)
		{
			...
			if ( $str = @mysql_error($this->dbh) )
			{
				...
				$this->show_errors ? trigger_error($str.'<br /><strong>Query: </strong>'.$query,E_USER_WARNING) : null;
				return false;
			}
	...
	include_once ($_SERVER['DOCUMENT_ROOT']."/treeline/includes/ezSQL.conn.php");
	
	//ini_set("display_errors",1);
	
	// Admin Database
	$database_admin = 'treeline2_admin';	
	//print "<!-- Host($host) Connect ($username, ".substr($password,0,2)."..., $database_admin, localhost) -->\n";
	//print "Host($host) Connect ($username, ".substr($password,0,2)."..., $database_admin, localhost) <br />\n";
	$db_admin = new ezSQL_mysql($username, $password, $database_admin, "localhost");
	//print "connected?<br>\n";
	
	// Live website database.
	//print "PW(".substr($password, 0, 2).")<br>\n";
	if (!$password) {
		$pw_query = "SELECT db_pass FROM clients WHERE client_id in (15, ".($client_id+0).") and db_pass>'' ORDER BY client_id DESC LIMIT 1";
		//print "$pw_query<br>\n";
		$password=$db_admin->get_var($pw_query);
	}
	//print "<!-- Host($host) Connect ($username, ".substr($password, 0, 4).", $database, $sqlserver) --> \n";
	//print "Host($host) Connect ($username, ".substr($password,0,2)."..., $database, $sqlserver) <br>\n";
	$db = new ezSQL_mysql($username,$password,$database,$sqlserver);
	
	if($DEBUG) {
		$db->debug_all = true;
		$db_admin->debug_all = true;
	}
?>
