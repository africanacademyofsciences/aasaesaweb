...
// Settings
	...
	$save_path = $uploader['save_path'].'/';
	$upload_name = "Filedata";
	$max_file_size_in_bytes = $uploader['file_size_limit'];
...
// Process the file
	/*
		...
		been saved.
	*/
	$original_file = $save_path.$file_name;
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $original_file)) {
		HandleError("File could not be saved: ". $original_file);
		exit(0);
	} else
	{
	
		define('VALID_INCLUDE', true);
		include($uploader['process_include']);
		
// Return output to the browser (only supported by SWFUpload for Flash Player 9)

		echo "File Received ". $original_file;
		exit(0);
	}
