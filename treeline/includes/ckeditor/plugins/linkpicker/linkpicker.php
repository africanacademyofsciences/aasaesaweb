...
if ($linktype=="image") redirect($imagepicker);
	...
	if ($_POST['source'] == "file") $linkfileimage = true;
	...
	$linkpadding = read($_POST, 'padding', '');
	//print "passed width($linkimagewidth)<br>\n";
	...
	if (!$linkimagewidth) {
		if (file_exists($_SERVER['DOCUMENT_ROOT']."/silo/images/".$linkimage)) {
			...
			if ($sz[0]>0) {
				...
				if (!$linkpadding) $linkpadding = $imagepaddingtop."px ".$imagepaddingright."px ".$imagepaddingbottom."px ".$imagepaddingleft."px";
			}
		}
	}
	else {
		if (!$linkpadding) $linkpadding = $imagepaddingtop."px ".floor($imagepaddingright/5)."% ".$imagepaddingbottom."px ".floor($imagepaddingleft/5)."%";
	}
...
$filedltype = read($_POST, 'filedltype', '');			// Download type
...
//print "file cat($filesection) subcat($filesubsection) intlink($intlink)<br>";
...
function setSlideshow() {
	...
	if (slideshow>0) {
		...
		else if (f.slidetype.value=="blueimp") setHrefLink("@@BLUEIMP-GALLERY-"+slideshow+"@@");
		showOkBtn();	
	}
	else hideOkBtn();
}
...
function setDLtype(t) {
	//alert(t);
	if (t) linkform.filedltype.value=1;
	else linkform.filedltype.value=0;
}
...
function onDialogEvent(e) {
	...
	if (e.name=="load") {
		editor = e.editor;
		var text, html, linktext;
		//linktext = editor.getSelection().getNative();
		//alert("load with gNtext("+linktext+")");
		//linktext = editor.getSelection().getStartElement().getOuterHtml()
		//alert("load with gsetext("+linktext+")");

		// This sometimes works as a way to get the image html
		//var element = editor.getSelection().getSelectedElement()
		//if (element) html = element.getHtml();
		//alert("load with text("+text+") html("+html+")");
		
		//html = editor.getSelection().getNative().createRange().htmlText;
	...
	if (e.name=="ok") {
		if (!inserted) {
			...
			if (linkform.linkhref.value) {
				...
				if (lfilename && linkform.filedltype.value==1) {
					//alert("Set up a download block for file("+lfilename+")");
					//alert("filedl is(<?=$filedltype?>)");
					//alert("insert(@@"+linkform.linkhref.value+"@@)");
					llink = "@@FILE"+linkform.linkhref.value+"@@";
				}
				else {
					llink = '<a href="';
					...
					if (limage) {
						...
						if (caption || credit>0) {
							...
							html = "<div class=\"img_block img_"+(linkform.img_align.value)+"\" style=\"width:<?=($linkimagewidth)?>;padding:<?=($linkpadding)?>;\" >"+html;
							//html = html + limage;
...
<form method="post" id="linkform" action="">
    ...
    <input type="hidden" name="filedltype" value="<?=$filedltype?>" />
...
if($_POST || $linktype=="icon"){
	...
	switch($linktype){
		...
		case "icon": 
			...
			$prefix = "fa";
			if (substr($_GET['name'], 0, 3) == "ion") $prefix = "ion";
			?>
            <p>Icon selected, press ok to add this icon to your page.</p>
            <i class="<?=$prefix?> <?=$_GET['name']?>"></i>
            <script type="text/javascript">
			showOkBtn();
			</script>
            <?php
		...
		case 'file':  
			...
			if( empty($error_list) && ($intlink>'') ){
                ...
                <input type="hidden" name="linktype" value="<?=($linkfileimage?"embed-image":$linktype)?>" />
                <input type="hidden" name="source" value="<?=($linkfileimage?"file":"")?>" />
                <input type="hidden" name="linktext" value="<?=$linktext?>" />
                <input type="hidden" name="newlink" value="" />
                ...
                <input type="hidden" name="img_style" value='<?=$linkimagestyle?>' />
                <input type="hidden" name="img_alt_text" value='<?=$linkimagealt?>' />
                <input type="hidden" name="width" value="<?=$linkimagewidth?>" />
                <input type="hidden" name="padding" value="<?=$linkpadding?>" />
                ...
                <?php } ?>
                <fieldset title = "Type of file link">
                    <label for="f_dltype">Use download block:</label>
                    <input type="checkbox" class="checkbox" name="filedltype" id="f_dltype" value="1" <?=($filedltype==1?'checked="checked"':'')?> onchange="setDLtype(this.checked)" />
                </fieldset>
                </form>
					...
					if ($slidehtml) {
						...
						<fieldset id="f_slidetype">
							...
							<select name="slidetype" onChange="setSlideshow();">
                                                            ...
                                                            <option value="blueimp">Blueimp Owl</option>
							</select>
						</fieldset>
						<?php
					}
				...
				if ($tl_admin) {
                    ...
                    <li>
                        <form method="post" id="slidelinkform">
                            <input type="hidden" name="linktype" value="slide" />
                            <input type="hidden" name="linktext" value="[Slideshow placeholder]" />                
