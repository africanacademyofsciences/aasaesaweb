...
class Securimage {
  ...
  function output()
  {
    ...
    $test ? print "header($header)<br>\n":header($header);
	$header = "Last-Modified: " . gmdate("D, d M Y H:i:s") . "GMT";
    $test ? print "header($header)<br>\n":header($header);
	$header = "Cache-Control: no-store, no-cache, must-revalidate";
    $test ? print "header($header)<br>\n":header($header);
	$header = "Cache-Control: post-check=0, pre-check=0";
    $test ? print "header($header, false)<br>\n":header($header, false);
	$header = "Pragma: no-cache";
    $test ? print "header($header)<br>\n":header($header);

	if ($test) print "got image type(".$this->image_type.")<br>\n";
    switch($this->image_type)
    {
      case SI_IMAGE_JPEG:
        $header = "Content-Type: image/jpeg";
	    $test ? print "header($header)<br>\n":header($header);
        if (!$test) imagejpeg($this->im, null, 90);
        break;

      case SI_IMAGE_GIF:
        $header = "Content-Type: image/gif";
	    $test ? print "header($header)<br>\n":header($header);
        if (!$test) imagegif($this->im);
        break;

      default:
        $header = "Content-Type: image/png";
	    $test ? print "header($header)<br>\n":header($header);
        if (!$test) imagepng($this->im);
        break;
    }
