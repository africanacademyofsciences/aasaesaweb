<?php
	...
	$js = array('jquery','preloadCssImages','page_functions','lytebox','../store/behaviour/store_equalheightblocks'); // all atatched JS behaviours
    ...
    <select id="f_option" name="id">'.$varTxt.'</select>';
            ...
            */
            ...
            <div id="product" class="hlisting">
                ...
                if ($mainImg) { 
                    ?>
                    <div class="productImage" id="mainImage" style="background-image: url(/store/images/<?= $product->product_id ?>/<?= $mainImg ?>_large.<?=$mainImgExt?>)">
                ...
                <div id="productDetail">
                    ...
                    if( count($images)>1 ){
                        foreach($images as $img_id => $values){
                            ...
                            if ($img_id != $mainImg) {
                                $loc = $_SERVER['DOCUMENT_ROOT'] .'/store/images/'. $product->product_id .'/'. $img_id .'.'.$values['extension'];
                                if( file_exists($loc) ){
                                    //print "found for file($loc)<br>\n";
                                    ?>	
                                    <div class="smallImageHolder">						
                                        <div id="image<?=$img_id?>" class="productImage productImageSmall" style="background:url(/store/images/<?= $product->product_id ?>/<?= $img_id ?>_thumb.<?=$values['extension']?>) no-repeat 50% 50%">
                                            <a class="magnify" href="/store/images/<?= $product->product_id ?>/<?= $img_id ?>.<?=$values['extension']?>" rel="lytebox[product-<?= $product->product_id ?>]" target="_blank" title="<?= $product->title?>: <?= $values['caption'] ?>">Click to view large image</a>
                                        </div>
			...
			foreach($products as $item) {   
				...
				<li id="<?=( ($i==1 && $currentPage==1 && $store->config["store_use_featured"])?'featured':'prod'.$i)?>" class="productInfo<?=(in_array($i, $lineEnds)?' last':'')?>" >
					...
					if($i==1 && !$resetPriority && $store->config["store_use_featured"]){ 
						...
						if ($mainImg) { 
							?>
							<div class="productImage" style="background:url(/store/images/<?= $item->product_id ?>/<?= $mainImg ?>_small.<?=$mainImgExt?>) no-repeat 50% 50%">
					...
					// Or just a standard product layout?
						...
						if ($mainImg) { 
							?>
							<div class="productImage productImageSmall" style="background:#CCC url(/store/images/<?= $item->product_id ?>/<?= $mainImg ?>_small.<?=$mainImgExt?>) no-repeat 50% 50%">
								<a href="<?= $productLink ?>" class="magnify">View '<?= $item->title?>' details</a>
							</div>
							<?php
						}
...
<div id="secondarycontent">
	<? include($_SERVER['DOCUMENT_ROOT'] .'/store/snippets/panel.basket.php') ?>
